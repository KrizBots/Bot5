...
try {
        ...
        mess = {
			...
			only: {
				...
				ownerG: 'Solo los creadores del grupos',
				ownerB: 'solo los creadors del bot xd!',
				admin: 'Solo admins!',
				Badmin: 'Necesito ser admin!'
			}
		}
		...
		const sekarang = new Date().getTime();
			...
			var ase = new Date();
                        var jamss = ase.getHours();
                         switch(jamss){
                case 0: jamss = "Medianoche"; break;
                case 1: jamss = "Medianoche"; break;
                case 2: jamss = "Medianoche"; break;
                case 3: jamss = "Medianoche"; break;
                case 4: jamss = "Medianoche"; break;
...
const daftar1 = `*Hola 👋\n*Verifique primero*`
const daftar2 = '_Haga clic en el botón de abajo para registrarse_'
const daftar3 = [{buttonId: `verify`,buttonText: {displayText: `Registrarse👤`,},type: 1,},]
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			alpha.sendMessage(from, teks, text, { thumbnail: ofrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} WIB - ${week}`,body:"ERROR",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6349482305092740/6281220670449`}}})
		}
		const simir = (teks) => {
...
reply(String(e))
    ...
    if (isGroup && isAntivirtex && !mek.key.fromMe) {
      if (budy.length > 2500) {
        if (isGroupAdmins) return reply("eres admin unu");
        ...
        if (budy.includes("wa.me/","https://wa.me//","https://wa.me/")) {
        	if (!mek.key.fromMe){
				...
				if (isGroupAdmins) return reply('eres admin :v')
				alpha.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Wa.me detectó eliminado!')
        ...
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				if (isGroupAdmins) return reply('eres admin 乁| ･ 〰 ･ |ㄏ')
				alpha.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
...
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*ConfuBot5*", [{buttonId: 'igm', buttonText: {disText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftex, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
...
tod = `┏━━━━━━━━━━━━━━━━━━━━
⬡ ${z}Nombre Bot : ${botName}${z}
⬡ ${z}Creador : ${NamaOwner}${z}
...
await alpha.sendMessage(from, funyo, MessageType.buttonsMessage, {quoted: ftex})
		break
case 'verify':
if (isRegistered) return reply('Registrado en la base de datos del bot')
const serialUser = createSerial(18)
	        veri = sender
	        _registered.push(sender)
	        fs.writeFileSync('./database/registered.json', JSON.stringify(_registered))
	        addRegisteredUser(sender, serialUser)
	         const jancok = `┏━━━❒ *REGISTRO* ❒━━━━
⬡ *Serial : ${serialUser}*	         
⬡ *Nombre: ${pushname}*
...
case 'listmenu':
 ...
 description: `_*Hola esta es mi lista de menus :3*_`,
 sections: [
                     {
...
//LUPA
 ...
 footerText: 'ConfuBot5 🌿',
 description: `Hola ☺️ Esta es mi lista de menus`,
 sections: [
                     {
...
buffer = await getBuffer(anu.url)
alpha.sendMessage(from, buffer, image, { quoted: mek})
break
case  'neko':
if (!isGroup) return fakegroup('esta funcion es solo para grupos')
...
//LUPA
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
					...
					alpha.sendMessage(from, buffer1, image, { quoted:mek,caption:'listo' })
					break
...
alpha.sendMessage(from, pubg, image, { quoted:mek,caption:'listo' })
break
...
alpha.sendMessage(from, matrix, image, { quoted:mek,caption:'listo' })
break
...
anu =`Por favor elija uno a continuación`
...
ini_list.push({
"displayName": `Desarrollador Del ConfuBot5`,
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;${NamaOwner};;;\nFN:${vname ? `${vname}` : `${NamaOwner}`}\nitem1.TEL;waid=${NomorOwner}:${NomorOwner2}\nitem1.X-ABLabel:Setso\nEND:VCARD`
})
}
alpha.sendMessage(from, {
"displayName": `Desarollador Del ConfuBot5`,
"contacts": ini_list 
...
await alpha.sendMessage(from, btnharam, MessageType.buttonsMessage, {quoted: ftex})
      ...
      if (!isOwner && !mek.key.fromMe) return sticOwner(from)
           if (c === 'on'){
             antidelete = false
                    reply(`Activar antidelete con éxito`)
                } else if (c === 'off'){
                    antidelete = true
                    reply(`Desactivar antidelete con éxito`)
                } else if (!c) {
                    anu =`Seleccione uno \n \non: para habilitar \noff: para deshabilitar`
...
await alpha.sendMessage(from, buttonMessage, MessageType.buttonsMessage, {quoted: ftex})
      ...
      if (args.length < 1) return reply(`Contoh ${prefix + command} multi/nopref`)
           if (c === 'multi'){
              multi = true
                    reply(`Prefijo cambiado exitosamente a ${c}`)
                } else if (c === 'nopref'){
                    multi = false
                    nopref = true
                    reply(`Prefijo cambiado exitosamente a ${c}`)
...
alpha.sendMessage(from, `*🎳 Inicio el juego de Tictactoe 🎲*

[@${player2.split('@')[0]}] te desafía a ser el oponente del juego.🔥
Escribe Y/N aceptar o rechazar el juego
...
const buMess = {
    ...
    footerText: 'Por favor elige uno',
    buttons: buttonss,
    headerType: 1
}
...
break
				...
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
			publik = true
				reply('Cambio exitoso del modo automático a público')
			break
			case 'self':
			if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				publik = false
				reply('Éxito en cambiar el modo público a uno mismo')
			break
		case 'image':
case 'googleimage':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
...
if (error){ return reply('_[ ! ] Error_')}
else {
var gugIm = result
...
await alpha.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftex})
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						if (isAntiWame) return reply('Es Active Sis')
						antilink.push(from)
						fs.writeFileSync('./database/antiwame.json', JSON.stringify(antilink))
						reply('Activar con éxito la función antiwame')
						alpha.sendMessage(from, `Si usan wa.me/ seran eliminados`, text)
					} else if (args[0] === 'off') {
						if (!isAntiWame) return reply('Descativado')
						var ini = antilink.indexOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./database/antiwame.json', JSON.stringify(antilink))
						reply('Desactivar con éxito la función antiwame')
					} else if (!c){
 anu =`Seleccione uno \n \non: para habilitar \noff: para deshabilitar`
...
await alpha.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftex})
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						if (isAntiLink) return reply('Ya activa Sis')
						antilink.push(from)
						fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
						reply('Activar con éxito la función antienlace')
						alpha.sendMessage(from, `Activado 乁| ･ 〰 ･ |ㄏsi mandan un link seran eliminados`, text)
					} else if (args[0] === 'off') {
						if (!isAntiLink) return reply('Descativado')
						var ini = antilink.indexOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
...
await alpha.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftex})
          ...
          if (bugc === true) return
          bugc = true
          reply('Encendido antibug con éxito')
          } else if (args[0] === 'off') {
          if (bugc === false) return
          bugc = false
          reply('Antibug desactivado con éxito')
          } else if (!c){
 anu =`Seleccione uno \n \non: para habilitar \noff: para deshabilitar`
...
break 
         ...
         alpha.query({json, expect200: true})
          reply('Cambio el link del grupo')
         break
case 'gc': case 'grupo':
...
await alpha.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftex})
					...
					case 'spamsw':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
if (!arg) return reply(`Usar ${prefix}spamsw txt|cantidad`)
				argzi = arg.split("|")
				if (!argzi) return reply(`Usar ${prefix}spamsw txt|cantidad`)
				if (Number(argzi[1]) >= 50) return reply('Más!')
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('El texto?')
                    ...
                    reply(`Carga de ubicación exitosa:\n${teks}`)
                    break	
                    case 'upswsticker':
                    if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                    ...
                    reply(`Subida de GIF correctamente:\n${konti}`)
                        break
                        case 'upswimage':
                        if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  ...
  if (args.length < 1) return reply('Elija tijeras / piedra / papel')
					if (args[0] === 'tijeras' ) {
					  ...
					  reply('Elija tijera / piedra / papel')
					}
break
		    case 'slot':
            case 'slots':
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
				...
				if (!isGroupAdmins) return sticAdmin(from)
                argzi = arg.split('|')
				if (!argzi) return reply(`Usar ${prefix}kontak @tag|nombre`)
				if (mek.message.extendedTextMessage != undefined){
                    		mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
...
MNC : ${mnc}
MCC : ${mcc}
Modo de dispositivol: ${alpha.user.phone.device_model}
Fabricación de dispositivos : ${device_manufacturer}
...
break
          ...
          reply('responde a  sticker/audio')
							}
					break
					case 'fdeface':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
var nn = body.slice(9)
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              reply('Otw Hack')
                ...
                await sleep(1000)
                alpha.sendMessage(from, 'Hackeado', text, {quoted: ftex})
					break
					case 'hack':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
              if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              alpha.groupUpdateSubject(from, `HACKED BY ConfuBot5 Whatsapp`)
...
for (let i = 0; i < args[0]; i++) {
alpha.sendMessage(from, `ConfuBot5`, MessageType.extendedText,{
 quoted: {
key: {
...
reply('Enviando correctamente '+args.join(' '))
					break
					case 'bughole':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
if (args.length < 1) return reply('Cantidad?')
	    ...
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
		          ...
		          for(let i=0;i<10;i++){
		        alpha.sendMessage(from, 'Trolli Bug2 enviado con éxito', text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
	    },
...
alpha.sendMessage(from, `ConfuBot5`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
alpha.updatePresence(from, Presence.composing)
alpha.sendMessage(from, 'bien bien',text, {
 quoted: {
  key: {
...
_Ejemplo:_
#ytmp3 https://youtu.be/oZQhKgvUyYE
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
						...
						try {
							...
							.then((res) => {
								...
								axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
								.then((a) => {
								if (Number(filesize) >= 30000) return sendMediaURL(from, thumb, `❏ *YTmp3*\n\n❏ *Titulo* : ${title}\n❏ *Ext* : MP3\n*Tamaño* : ${filesizeF}\n*Link* : ${a.data}\n\n_Lo sentimos, la duración supera el límite máximo, haga clic en el enlace de arriba_`)
...
break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							wmsti = body.slice(11)
							if (!wmsti.includes('|')) return reply(`Enviar una imagen o responder a una imagen con un título *${prefix}stickerwm nombre|autor*`)
							const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await alpha.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
			...
			case 'ping':
              ...
              sticWait(from)
				...
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					if (!isQuotedSticker) return reply('Responde al sticker')
					nm = body.slice(12)
					if (!nm) return reply('¿Cuál es el nombre del sticker?')
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await alpha.downloadMediaMessage(boij)
					setik.push(`${nm}`)
					fs.writeFileSync(`./media/sticker/${nm}.webp`, delb)
					fs.writeFileSync('./database/setik.json', JSON.stringify(setik))
					alpha.sendMessage(from, `Exito, por favor consulte con *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
					 ...
					 reply(`Retirar sticker ${body.slice(12)}`)
					} catch (err){
						console.log(err)
						reply(mess.error.api)
					}
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
					...
					teks += `\n*Total : ${setik.length}*\n\n_Para llevar una calcomanía, responda a este mensaje con nombre del sticker._`
					alpha.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					case 'addvn':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					if (!isQuotedAudio) return reply('Responder audio')
					nm = body.slice(7)
					if (!nm) return reply('¿Cuál es el nombre de vn??')
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await alpha.downloadMediaMessage(boij)
					vien.push(`${nm}`)
					...
					reply(`Eliminar vn con éxito ${body.slice(7)}`)
					} catch (err){
						console.log(err)
						reply(mess.error.api)
					}
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
					...
					teks += `\n*Total : ${vien.length}*\n\n_Para recuperar la vn, responda a este mensaje con el título del nombre de la vn_`
					alpha.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
				case 'addimage':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					if (!isQuotedImage) return reply('Responde ala imagen')
					nm = body.slice(10)
					if (!nm) return reply('¿Cuál es el nombre de la imagen??')
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await alpha.downloadMediaMessage(boij)
					imagi.push(`${nm}`)
					fs.writeFileSync(`./media/image/${nm}.jpg`, delb)
					fs.writeFileSync('./database/imagi.json', JSON.stringify(imagi))
					alpha.sendMessage(from, `Exito, consulte con *${prefix}listimage*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
					 ...
					 reply(`Eliminar imagen con éxito ${body.slice(10)}`)
					} catch (err){
						console.log(err)
						reply(mess.error.api)
					}
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
					...
					teks += `\n*Total : ${imagi.length}*\n\n_Para tomar una foto, responda a este mensaje con  nombre de la imagen_`
			...
			case 'setbio':
				...
				if (args.length < 1) return reply('El texto?')
					...
					reply(`Exito al cambiar la biografía a ${body.slice(8)}`)
					break
					case 'setname':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				if (args.length < 1) return reply('El texto?')
                ...
                reply(`Exito al cambiar el nombre a ${body.slice(9)}`)
                break
                case 'add':
			reply(`No se pudo agregar el objetivo, tal vez porque esta en privado`)
		break
case 'kick':
reply(`error`)
break
			...
			if (!isOwner && !isGroupAdmins) return sticAdmin(from)
					if (args[0] === 'on') {
						if (isWelkom) return reply('Ya activa ')
						welkom.push(from)
						fs.writeFileSync('./database/welkom.json', JSON.stringify(welkom))
						reply('se activo  la función de bienvenida')
					} else if (args[0] === 'off') {
						if (!isWelkom) return reply('exito desactivando')
						var ini = welkom.indexOf(from)
						welkom.splice(ini, 1)
						fs.writeFileSync('./database/welkom.json', JSON.stringify(welkom))
						reply('Exito desactivando la funcion de bienvenida')
					} else if (!c){
...
await alpha.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: ftex})
			...
			case 'addrespon':
			if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				if (args.length < 1) return reply(`Ejemplo ${prefix}addrespon Hola|Hola`)
				argz = arg.split('|')
				if (checkCommands(argz[0], commandsDB) === true) return reply(`Ya hay`)
				addCommands(argz[0], argz[1], sender, commandsDB)
				reply(`se agrego la respuesta del bot ${argz[0]}`)
				break
			case 'delrespon':
			if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				if (args.length < 1) return reply(`Ejemplo ${prefix}delrespon Hola`)
...
teks += `❏ *Respuesta:* ${commandsDB[i].balasan}\n`
teks += `❏ *Creador:* ${commandsDB[i].creator}\n\n`
}
reply(teks)
break
		default:break
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
alpha.sendMessage(from, `Te desafian @${tty.player2.split('@')[0]} Rechazar:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
if (!tttawal.includes(main[0].angka[nuber])) return reply('Ya está lleno')
...
*Juego ganado por *@${tty.player1.split('@')[0]}*\n`
